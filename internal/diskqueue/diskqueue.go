...
func New(name string, dataPath string, maxBytesPerFile int64,
	...
	minCompressSize int32) Interface {
	d := diskQueue{
		name:              name,
		dataPath:          dataPath,
...
func (d *diskQueue) readOne() ([]byte, error) {
	...
	compress, err := d.reader.ReadByte()
	if err != nil {
		d.readFile.Close()
		d.readFile = nil
		return nil, err
	...
	if CompressSnappy == compress {
		v, er := snappy.Decode(nil, readBuf)
		if nil != er {
			d.logf(ERROR, "invalid compress msg: %v", er)
			return readBuf, nil
		}
		d.logf(DEBUG, "compress msg ratio = %v", len(readBuf)/len(v))
		return v, er
	}
	return readBuf, nil
}
...
func (d *diskQueue) writeOne(data []byte) error {
	...
	compress := NoCompress
	if d.minCompressSize > 0 && d.minCompressSize < dataLen {
	...
	d.writeBuf.Reset()
	err = d.writeBuf.WriteByte(compress)
	if err != nil {
		return err
	}
